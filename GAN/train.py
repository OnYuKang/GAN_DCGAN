...
"""
This file trains the GAN Model.
"""
...
# make GAN instance
Gan = gan.gan(config.num_classes)

sample = Gan.generate_sample(Z,config.batch_size, config.z_dim, IS_TRAINING)
...
with tf.control_dependencies(discriminator_update_ops):
    train_discriminator = tf.train.AdamOptimizer(learning_rate=0.0003,beta1=0.5).minimize(d_loss,
                                 var_list=discriminator_variables)

with tf.control_dependencies(generator_update_ops):
    train_generator = tf.train.AdamOptimizer(learning_rate=0.0002,beta1=0.5).minimize(g_loss,
...
try:
    for epoch in range(0,config.num_epochs):
        ...
        for next_idx ,X_mb in batched_images(config.batch_size,DB_PATH, 1 ):            
            ...
            _, d_summaries, d_loss_step = sess.run([train_discriminator, d_loss_summary, d_loss], feed_dict={X: X_mb, Z: noise, IS_TRAINING : True})
            _, g_summaries, g_loss_step = sess.run([train_generator, g_loss_summary, g_loss], feed_dict={X: X_mb, Z: noise, IS_TRAINING : True})
        ...
        samples = sess.run(sample, feed_dict={Z:np.random.uniform(-1., 1., size=[128, config.z_dim]), IS_TRAINING:False})
        ...
        plt.savefig('out/final_{}.png'.format(str(epoch).zfill(3)), bbox_inches='tight')
        plt.close(fig)    
        ...
        saver.save(sess, "./saved_models/lsun-epoch%d.ckpt" % (epoch,))
